...
export async function generateWorkflowSource(
  ...
  const prompt = [
    ...
    options.workflowKind === "build"
            ...
            : `The workflow is SELF-DRIVEN: declare export const workflowKind = "test-workflow-driven" at the top and make the default-exported function RUN THE TESTS ITSELF and return nothing. It runs ONCE PER WORKTREE (baseline then candidate) with the SAME source; you cannot tell which side you are on. Run the tests with context.process.run / context.adapters.ctest.run / context.validator.assertFile, then declare expectations with ctx.expect(name, value) (equal across sides) or ctx.expect(name, relation, value) where relation is "baseline-greater" | "baseline-less" | "not-equal" | "both-matches" (with a ^regex$ pattern as 4th arg for both-matches). Values are THIS side's observed values. Declare the SAME expectations in the SAME ORDER on both runs — never branch on side or environment before an expectation. Never return a declarative TestWorkflow object.`,
